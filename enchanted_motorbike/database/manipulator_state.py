...
class ManipulatorStatesRepository:
    ...
    async def history(
        ...
        items = (
            await self.__collection.find(request)
            .sort("made_at", 1)
            .to_list(length=None)
        )
    ...
    @classmethod
    def __history_request(
        cls, after: datetime | None, before: datetime | None
    ) -> dict | None:
        conditions = {}
    ...
    def __parse_list(cls, items: list) -> list[ManipulatorStateDecision]:
        return [parse_obj_as(ManipulatorStateDecision, item) for item in items]
